      ...
      {computed && result && (
        <>
          ...
          <div className="text-xs text-muted-foreground border rounded-md px-3 py-2 bg-muted/30">
            Parsed: <strong>{result.shifts.length}</strong> total shifts ·{' '}
            <strong>{result.byProvider.size}</strong> providers with shifts.
            {result.shifts.length === 0 && (
              <span className="text-amber-700"> No shifts produced — check that the Jotform CSV has a "For which month" column matching <code>June</code>, and that the "What days and times…" columns contain rows in the expected format.</span>
            )}
          </div>
