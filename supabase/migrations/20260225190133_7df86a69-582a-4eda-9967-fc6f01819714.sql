...
-- Function: auto-create collaborative agreement when an agreement_creation transfer task is completed
...
BEGIN
  ...
  -- Create the new collaborative agreement
  INSERT INTO public.collaborative_agreements (
    ...
    COALESCE(v_transfer.effective_date, CURRENT_DATE),
    'in_progress',
...
-- Create the trigger
DROP TRIGGER IF EXISTS trg_auto_create_agreement_on_task_complete ON public.agreement_tasks;
CREATE TRIGGER trg_auto_create_agreement_on_task_complete
  AFTER UPDATE ON public.agreement_tasks
  FOR EACH ROW
  WHEN (OLD.status IS DISTINCT FROM NEW.status AND NEW.status = 'completed')
  EXECUTE FUNCTION public.auto_create_agreement_on_task_complete();
