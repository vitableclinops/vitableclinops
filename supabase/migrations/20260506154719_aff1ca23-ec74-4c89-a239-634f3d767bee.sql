CREATE TABLE IF NOT EXISTS public.publish_status (
  ...
  homebase_posted boolean NOT NULL DEFAULT false,
  homebase_posted_at timestamptz,
  homebase_posted_by uuid,
  ehr_posted boolean NOT NULL DEFAULT false,
  ehr_posted_at timestamptz,
  ehr_posted_by uuid,
  notes text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (provider_id, month_key)
);
...
CREATE INDEX IF NOT EXISTS idx_publish_status_month ON public.publish_status(month_key);
CREATE INDEX IF NOT EXISTS idx_publish_status_provider ON public.publish_status(provider_id);