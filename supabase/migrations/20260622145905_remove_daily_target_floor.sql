alter table public.sla_daily
  ...
  add constraint sla_daily_daily_target_check check (daily_target >= 0);

alter table public.state_demand_targets
  drop constraint if exists state_demand_targets_daily_target_slots_check,
  add constraint state_demand_targets_daily_target_slots_check check (daily_target_slots >= 0);
