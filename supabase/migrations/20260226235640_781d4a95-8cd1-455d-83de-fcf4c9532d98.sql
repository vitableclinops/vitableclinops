...
DECLARE
  ...
  v_provider record;
  v_first_provider record;
  v_provider_name_display text;
  v_first_provider_id uuid;
BEGIN
  ...
  -- Resolve actual provider name and ID from affected_provider_ids
  v_provider_name_display := v_transfer.affected_provider_count || ' Provider(s)';
  v_first_provider_id := NULL;

  IF array_length(v_transfer.affected_provider_ids, 1) = 1 THEN
    -- Single provider: use their actual name and ID
    SELECT id, full_name INTO v_first_provider
    FROM public.profiles
    WHERE id = v_transfer.affected_provider_ids[1];
    
    IF v_first_provider IS NOT NULL THEN
      v_provider_name_display := COALESCE(v_first_provider.full_name, v_provider_name_display);
      v_first_provider_id := v_first_provider.id;
    END IF;
  ELSIF array_length(v_transfer.affected_provider_ids, 1) > 1 THEN
    -- Multiple providers: use first name + count
    SELECT id, full_name INTO v_first_provider
    FROM public.profiles
    WHERE id = v_transfer.affected_provider_ids[1];
    
    IF v_first_provider IS NOT NULL THEN
      v_provider_name_display := COALESCE(v_first_provider.full_name, '') || ' + ' || (array_length(v_transfer.affected_provider_ids, 1) - 1) || ' more';
      v_first_provider_id := v_first_provider.id;
    END IF;
  END IF;
